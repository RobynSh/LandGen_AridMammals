...
DartGenos_1Row <- "Ph_DArT_1Row.csv"
...
DartGenos_2Row <- "Ph_DArT_2Row.csv"
DartReadCounts <- "Ph_DArT_Counts.csv" 
...
# In this case, most populations are made up of 1-4 samples. I'll need to find another way to group samples. P. hermannsbergensis is a widespread generalist and individual samples are very spread out across the Pilbara. I'll group samples by the IBRA sub region instead (which isn't ideal as these are very large areas). Therefore, be on the lookout for strange FIS (Wahlund effect) and FST patterns (although, this should impact all SNPs equally, rather than creating outlier SNPs).   
...
gl.list.reports()
Pcoa.raw <- gl.print.reports(print_report = 1)$plot
...
# There are some vague regional groupings, however less than 1% of the variation is described by PC1 and PC2 (suggesting that there may not be much structure genetic structure present in P. hermannsbergensis. Note that the patterns shown in the unfiltered PCoA often do not change substantially after filtering (which is a good indication that you aren't biasing your results, just cleaning the data).
...
# The correlation plot shows that call rate is strongly correlated with total mean read count. This is not surprising, as a low read count results in missing data. Call rate is strongly correlated with FIS, FST, HWE and (to a lesser extent) heterozygosity. 
