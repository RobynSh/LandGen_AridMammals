...
DartGenos_1Row <- "Nt_DArT_1Row.csv"
...
DartGenos_2Row <- "Nt_DArT_2Row.csv"
DartReadCounts <- "Nt_DArT_Counts.csv" 
...
# In this case, N. timealeyi samples are very spread out across the Pilbara with most populations made up of only 1-3 individuals. I'll need to find another way to group samples. I'll group samples by the IBRA subregion instead (which isn't ideal as these are very large areas). Therefore, need to be on the lookout for strange FIS and FST patterns (although, this should impact all SNPs equally, rather than creating outlier SNPs).     
...
# The above figure shows that we've removed the individual calls that were most strongly impacted by read count. There is still a slight increase in the proportion of reference homozygotes and heterozygotes, and a decrease in the proportion of SNP homozygotes at the lower read counts. The stability of the called genotypes also starts to dissolve at around 150 reads. However, we have to be pragmatic as there is a trade off between removing the effect of read count and introducing too much missing data.
...
# Change the population sample size cutoff from ten, to nine. After removing individuals, the sample size of Fortescue dropped below the threshold, but I still think it's worth keeping it so that I have more populations for which to calculate our summary statistics.  
# Check if any of the pops have dropped below threshold sample size number
PopCutOff <- 9
...
# There are some obvious regional groupings. Note that the patterns shown in the unfiltered PCoA probably won't change substantially after filtering (which I take as a good indication that results aren't being biased, just cleaning some of the noise).
