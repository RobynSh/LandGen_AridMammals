...
DartGenos_1Row <- "Pc_DArT_1Row.csv"
...
if(BLAST == "Y") {
  ...
  SexChrom <- "X"
  ChromInfo.file <- "SNP_Filtering/Data/Mus.musculus_ChromInfo.csv"
  ContigCol_dart <- "Chrom_Mouse_v38p4"
  ContigPos_dart <- "ChromPos_Mouse_v38p4"
  ContigCol_ncbi <- "Chrom_Mouse_v38p4"
  ChromPos_ncbi <- "Start"
  EVal.col <- "AlnEvalue_Mouse_v38p4"
  Cnt.col <- "AlnCnt_Mouse_v38p4"
}
...
ind.metrics <- ind.metrics[ind.metrics$spp_code == Outfile_Desc, ]
...
# In this case, there's only one population with more than 4 samples. I'll need to find another way to group samples. P. chapmani is a habitat specialist, meaning that it is difficult to collect large numbers in single locations (individual samples are very spread out across the Pilbara). I'll group samples by the IBRA sub region instead (which isn't ideal as these are very large areas). Therefore, be on the lookout for strange FIS and FST patterns (although, this should impact all SNPs equally, rather than creating outlier SNPs).    
...
# In this data set, there appears to be quite a strong relationship between individual read count and the proportion of each called genotype. Of particular concern is that the proportion of heterozygotes continues to increase with read count (until about 60). After 60 reads, the genotypes become fixed for the reference or the SNP allele (indicating that these may represent paralogous regions). I'll have to be quite stringent with my individual read count filters in an attempt to minimise the effect of individual read count on the called genotype.  
...
# The above figure shows that we've removed many of the individual calls that were most strongly impacted by read count. There is still a trend towards an increase in the SNP allele with more reads (until any consistent pattern begins to fall apart at around 50). However, we have to be pragmatic as there is a trade off between removing the effect of read count and introducing too much missing data.
...
# Output figure
jpeg(filename = paste0(outpath, Outfile_Desc, "_Ind.ReadCount.jpg"), units =  "cm", width = 20, height = 28, res = 300)
...
# There is still a slight difference between the two data sets in this case, however, this is likely because mean read count thresholds will need to be more stringent than the default values used. For this reason I will use (stringent) locus level read count filters from now on.  
...
# Output plots
jpeg(filename = paste0(outpath, Outfile_Desc, "_Avg.Ind.ReadCount.jpg"), units =  "cm", width = 28, height = 20, res = 300)
ggarrange(smear.plot, labels = "a)", font.label = list(size = 8, face = "plain"), ggarrange(IndCR.plot, Samp.Map, ncol = 2, labels = c("b)", "c)"), font.label = list(size = 8, face = "plain"), widths = c(1/2, 1/2)), ncol = 1)
dev.off()
...
ChromInfo.ncbi <- read.csv(file = ChromInfo.file, stringsAsFactors = FALSE)
...
# Run the function to calculate population genetic metrics across the data set filtered for failed individuals.
...
gl.list.reports()
Pcoa.raw <- gl.print.reports(print_report = 1)$plot # change number to match temp file
...
# There are some obvious regional groupings. Some of the Hammersley samples are more similar to the Augustus sample (although this is likely because these individuals are at the eastern-most edge of the Hammersely sub region). Note that the patterns shown in the unfiltered PCoA probably won't change substantially after filtering (which I take as a good indication that results aren't being biased, just cleaning some of the noise).
...
# Output as jpeg
jpeg(filename = paste0(outpath, Outfile_Desc, "_Chrom.jpg"), units =  "cm", width = 20, height = 28, res = 300)
...
# of SNPs mapped to the reference genome.  
...
if(BLAST == "Y"){
  ...
  # Output figures
  jpeg(filename = paste0(outpath, Outfile_Desc, "_RawHistograms.Mapped-UnMapped.jpg"), units =  "cm", width = 28, height = 20, res = 300)
...
# Output figures
jpeg(filename = paste0(outpath, Outfile_Desc, "_Corr.QualFilt.jpg"), units =  "cm", width = 20, height = 20, res = 300)
corrplot.mixed(QualFilt.cor, upper = "circle", lower = "number", order = "FPC", tl.pos = "lt", tl.col = "black", tl.cex = 0.6, number.cex = 0.5, number.font = 1, cl.cex = 0.5, lower.col = "black", upper.col = c(viridis(n = 10, option = "D", begin = 0.1, end = 0.9), viridis(n = 10, option = "A", direction = -1, begin = 0.3)), outline = TRUE)
dev.off()

jpeg(filename = paste0(outpath, Outfile_Desc, "_SummStat.QualFilt.jpg"), units =  "cm", width = 28, height = 20, res = 300)
...
# Output figure
jpeg(filename = paste0(outpath, Outfile_Desc, "_Priv.Al.jpg"), units =  "cm", width = 28, height = 20, res = 300)
ggarrange(PrivAl.Plot.raw, PrivAl.Plot.filt)
dev.off()
...
if(BLAST == "Y"){
  ...
  snpgdsCreateGeno(paste0(outpath, Outfile_Desc, "_Mapped_gl.CR.RC.Rp.MAF.lD.gds"), genmat = genmat.m, sample.id = SampID.m, snp.id = SNP_ID.m, snp.chromosome = Chrom[!is.na(Chrom)], snp.position = ChromPos, snpfirstdim = TRUE)
  
  genmat.um <- t(as.matrix(gl.CR.RC.Rp.MAF.lD.Filt)[,is.na(Chrom)])
  SampID.um <- colnames(genmat.um)
  SNP_ID.um <- rownames(genmat.um)
  
  snpgdsCreateGeno(paste0(outpath, Outfile_Desc, "_UnMapped_gl.CR.RC.Rp.MAF.lD.gds"), genmat = genmat.um, sample.id = SampID.um, snp.id = SNP_ID.um, snpfirstdim = TRUE)

  gds.m <- snpgdsOpen(paste0(outpath, Outfile_Desc, "_Mapped_gl.CR.RC.Rp.MAF.lD.gds"))
  gds.um <- snpgdsOpen(paste0(outpath, Outfile_Desc, "_UnMapped_gl.CR.RC.Rp.MAF.lD.gds"))
  ...
  snpgdsCreateGeno(paste0(outpath, Outfile_Desc, "_gl.CR.RC.Rp.MAF.lD.gds"), genmat = genmat, sample.id = SampID, snp.id = SNP_ID, snpfirstdim = TRUE)
  gds <- snpgdsOpen(paste0(outpath, Outfile_Desc, "_gl.CR.RC.Rp.MAF.lD.gds"))
...
# Output figures
jpeg(filename = paste0(outpath, Outfile_Desc, "_Corr.FinalFilt.jpg"), units =  "cm", width = 20, height = 20, res = 300)
corrplot.mixed(Filt.cor, upper = "circle", lower = "number", order = "FPC", tl.pos = "lt", tl.col = "black", tl.cex = 0.6, number.cex = 0.5, number.font = 1, cl.cex = 0.5, lower.col = "black", upper.col = c(viridis(n = 10, option = "D", begin = 0.1, end = 0.9), viridis(n = 10, option = "A", direction = -1, begin = 0.3)), outline = TRUE)
dev.off()

jpeg(filename = paste0(outpath, Outfile_Desc, "_SummStat.FinalFilt.jpg"), units =  "cm", width = 28, height = 20, res = 300)
...
Pcoa.filt <- gl.print.reports(print_report = 3)$plot
...
# Output figure:
jpeg(filename = paste0(outpath, Outfile_Desc, "_PCoA.jpg"), units =  "cm", width = 20, height = 28, res = 300)
PCoA.Filt.plot
dev.off()
